...
export function SeriesManager({ series, updateSeriesList, error }: SeriesManagerProps) {
    ...
    updateSeriesList(
      series.map((serie) => {
        ...
        const updatedExercises = serie.exercises.map((ex) => ({
          ...
          restAfterSet:
            newType === 'superset'
              ? DEFAULT_REST_AFTER_SET_SUPERSET
              : ex.restAfterSet || DEFAULT_REST_AFTER_SET_STANDARD,
        }));
  ...
  const handleAddExercise = (seriesId: string, exercise: Exercise) => {
    const updatedSeries = series.map((s) => {
      ...
      const restAfterSet =
        s.type === 'superset' ? DEFAULT_REST_AFTER_SET_SUPERSET : DEFAULT_REST_AFTER_SET_STANDARD;
