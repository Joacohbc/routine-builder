                ...
                <div className="flex items-center gap-4 flex-1">
                  {ex.media.length > 0 ? (
                    <div className="w-16 h-16 rounded-xl bg-gray-100 dark:bg-surface-highlight overflow-hidden shrink-0">
                      {ex.media[0].type === 'image' && (
                        <img src={ex.media[0].url} className="w-full h-full object-cover" />
                      )}
                      {ex.media[0].type === 'youtube' && (
                        <img
                          src={ex.media[0].thumbnailUrl}
                          className="w-full h-full object-cover"
                        />
                      )}
                      {ex.media[0].type === 'video' && (
                        <video src={ex.media[0].url} className="w-full h-full object-cover" />
                      )}
                    </div>
                  ) : (
